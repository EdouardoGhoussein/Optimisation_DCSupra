function varargout = struct2var(varargin)
...
if nargout==0 %assign in caller
    ...
    if any(mask)
        str = sprintf('\t%s\n',names{mask});
        error('The following variables already exist in the caller workspace and will be overwritten:\n%s',str);
    end
    for ii=1:n
        assignin('base',names{ii},s.(names{ii}));
    end
