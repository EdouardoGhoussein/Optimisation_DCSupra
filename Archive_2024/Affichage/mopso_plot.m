function mopso_plot(plt_options)
    ...
    nbCtr = length(plt_options.Ctr_labels);
    rows = ceil(nbvar/1);
    ...
    figure(1)
    hold on
    plot(simout.Front_Pareto_Objectifs(2,:),simout.Front_Pareto_Objectifs(1,:),'or','Markersize',4); %pareto
    ...
    xlabel(plt_options.obj_labels(2))
    ylabel(plt_options.obj_labels(1))
    title(plt_options.title)
    grid on
    ax = gca; % Récupère l'axe courant
    ax.GridLineStyle = '-'; % Style de ligne continue (modifiable)
    ax.GridAlpha = 1; % Opacité maximale de la grille
    ax.LineWidth = 1; % Épaisseur des lignes de la grille
    
    figure(2)
    hold on
    for k = 1:nbvar
        subplot(rows,1,k)
        plot(simout.Front_Pareto_Objectifs(2,:),simout.Front_Pareto_Parametres(k,:),'o','Markersize',4)
        xlabel(plt_options.obj_labels(2))
    ...
    for k = 1:nbCtr
        subplot(rows,1,k)
        plot(simout.Front_Pareto_Objectifs(2,:),simout.Front_Pareto_Contraintes(k,:),'o','Markersize',4)
        xlabel(plt_options.obj_labels(2))
        ylabel(plt_options.Ctr_labels(k))
        %yline(plt_options.bounds(k,1), 'g')
        %yline(plt_options.bounds(k,2), 'g')
        legend('hide')
        grid on
