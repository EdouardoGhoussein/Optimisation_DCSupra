function [f,g,h] = J_obj2(X)
    ...
    Ic = 300; %Maximum sustained current through superconducting module
    T0 = 77; %Ambient temperature
    Tcarac = 38.7; %Copper characteristic temperature
    rho_Cu = (17e-9)*(T0-Tcarac)/(20+273.5-Tcarac); %Copper resistivity
    ...
    for k = 1:1:Swarm_size
        ...
        CAPEX.Cu = Cu_cost*V_Cu*m_Cu;
        CAPEX.Sc = Sc_cost*nt*Lt;
        %CAPEX_share = CAPEX_Cu/CAPEX_Sc;

        CAPEX.tot = CAPEX.Sc + CAPEX.Cu; %Capital Expenses
        

        OPEX.coil = mean(CPcoil)/eta;
        OPEX.Waste = mean(out.RScFCL.signals.values(end-N:end).*(out.i_ScFCL.signals.values(end-N:end).^2));
        %OPEX_share = OPEX_coil/Waste_out;
        
        OPEX.tot = OPEX.coil + OPEX.Waste; %Operating Expenses
    
        %[Imax, S_Cu*1e4, CAPEX_share, OPEX_share] %for diagnostic purposes
